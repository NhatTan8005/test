    ...
    public partial class FrmQLKH : Form
    {
        ...
        private void btnXOA_Click(object sender, EventArgs e)
        {
            for (int i = 0; i < dgvKH.Rows.Count; i++)
            {
                ...
                {
                    if (MessageBox.Show("Bạn có chắc muốn xóa khách hàng: " + dgvKH.Rows[i].Cells["TENKH"].Value.ToString() + " ?", "Xác Nhận Xóa !!", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        delete(dgvKH.Rows[i].Cells[1].Value.ToString());
                    }
                }

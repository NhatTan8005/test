    ...
    public partial class frmQLNV : Form
    {
       ...
       // public frmNV nv;
        ...
        private void btnXOA_Click(object sender, EventArgs e)
        {
            ...
            {
                if (dgvNV.Rows[i].Cells[0].Value != null)
                {
                    if (MessageBox.Show("Bạn có chắc muốn xóa Nhân viên: " + dgvNV.Rows[i].Cells["TENNV"].Value.ToString() + " ?", "Xác Nhận Xóa !!", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        delete(dgvNV.Rows[i].Cells[1].Value.ToString());
                    }
                }

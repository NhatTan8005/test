    ...
    public partial class frmHOADON : Form
    {
        ...
        private void btnXOA_Click(object sender, EventArgs e)
        {
            for (int i = 0; i < dgvHOADON.Rows.Count; i++)
            {
                ...
                {
                    if (MessageBox.Show("Bạn có chắc muốn xóa hóa đơn: " + dgvHOADON.Rows[i].Cells["SOHD"].Value.ToString() + " ?", "Xác Nhận Xóa !!", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        delete(dgvHOADON.Rows[i].Cells[1].Value.ToString());
                    }
                }

    ...
    public partial class frmQLTOUR : Form
    {
       ...
       // private DateTime Mngayketthuc;
        ...
        private void btnmoi_Click(object sender, EventArgs e)
        {
            ...
            {
                if (dgvTOUR.Rows[i].Cells[0].Value != null)
                {
                    if (MessageBox.Show("Bạn có chắc muốn xóa Tour: " + dgvTOUR.Rows[i].Cells["MATOUR"].Value.ToString() + " ?", "Xác Nhận Xóa !!", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        delete(dgvTOUR.Rows[i].Cells[1].Value.ToString());
                    }
                }
